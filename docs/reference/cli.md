...
- **The UI has no view for `show leases`, `show device`, `show ipv6`, `show time`, `show storage`, or a pending confirm window.** Those exist only on the CLI. `show counters` now has one (`/counters`, the same opcode and the same four states), and `/policy` shows what `fd` has loaded per zone — but not the rules, which nothing can ask the daemon for; `show policy` reads the source instead and says so.
