...
| `show counters [<name>]` | What each `count <name>` in the loaded policy has counted, per zone, read out of that zone's `fwl_counters_<zone>` map. |
...
| Command | |
|---|---|
| `show policy [zone]` | Two answers on one screen. **LOADED** is what `fd` has in the packet path, zone by zone in policy order — the rules the compiler wrote into the bundle manifest, captured by the load that attached the programs. **SOURCE** is the `.fw` on disk, block by block, numbered; those numbers are what `no rule` takes and they restart in each `@xdp` block. Below both, a verdict on whether the file is still the policy that is running. The `MATCHES` column marks the statements that act on **every** packet, because nothing below one of those can ever match. |
...
`show policy` reports both halves and compares them. The **LOADED** table comes from `fd` over opcode 13 and describes the packet path; the **SOURCE** table is the file. The verdict under them is three-valued and none of the three is a guess: the file is the loaded policy, the file has been **edited since this policy was compiled** (the box is enforcing the older one), or this box **cannot tell** — because the bundle records no source, or because the file could not be read. A box that answered "matches" for a comparison it never made would be the worse failure, so it does not.

The LOADED rows carry no position. `no rule` numbers the SOURCE statements — the compiler folds some of them away and the default action is not among them at all — so a position taken from a loaded row would delete a statement nobody was looking at.
...
- **`show firewall`, `show firewall rules` and `clear counters` do not exist.** All three addressed the v0.1 single-program datapath: a separate `counters` map keyed by match tier rather than by rule, which no v0.4 bundle has. On a real box `show firewall` printed a fixed `default_action drop / active_table 0 / conntrack disabled / rule_count 0` regardless of the policy, and the other two were refused by the daemon. They are removed rather than left answering. `show counters` is back, reading the map the datapath actually writes (`fwl_counters_<zone>`) and naming each slot from the compiler's own `// fwl_counter_table:` block — a different map, a different opcode, and a different answer. There is still no per-rule hit count, and that is now a deliberate omission rather than a missing input: the bundle carries per-zone rule metadata and `show policy` renders it, but a rule and a counter are joined by nothing in that answer. Pairing them by position is exactly what the retired `kGetRules` did — it walked a rule list and a counter array in step while the datapath keyed them by match tier — so `show counters` answers by NAME and `show policy` answers by ORDER, and neither offers an index the other could be indexed with.
...
- **The UI has no view for `show leases`, `show device`, `show ipv6`, `show time`, `show storage`, or a pending confirm window.** Those exist only on the CLI. `show counters` now has one (`/counters`, the same opcode and the same four states), and `/policy` shows what `fd` has loaded per zone, rules included — the same opcode-13 answer `show policy` renders, so the two surfaces agree by construction. What `/policy` does **not** show is the drift verdict, because computing it means reading the `.fw` on disk and that page's whole claim is the packet path; it prints the loaded policy's source name and digest instead, and names `einheit-f show policy` as the thing that makes the comparison.
