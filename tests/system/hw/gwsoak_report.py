...
def main() -> int:
  ...
  if len(blocks) > 1:
    print()
    print("Tier 2 rate limiting is NOT witnessed here: this soak's "
          "Tier 2 zones")
    print("carry no limiter, so the flood stays on the Tier 1 zones, "
          "where it is")
    print("measured exactly as it was before the boundary. The policy "
          "was written")
    print("that way because a Tier 2 `rate_limit(...)` emitted a "
          "constant false until")
    print("2026-08-19 (l7_01_tier2_rate_limit_gap). That is fixed; "
          "exercising it here")
    print("needs a policy change, so a run predating one still reports "
          "no witness.")
  print()
