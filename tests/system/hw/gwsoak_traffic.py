...
Three roles, one program, selected with `--role`:
  ...
  t2inside One Tier 2 zone's workload (epoch 2). The same shape as
           ...
           `rate_limit(...)` emitted a constant false when this was
           written — see l7_01_tier2_rate_limit_gap, fixed 2026-08-19 —
           so flooding a zone that could not limit would have measured
           nothing; the zones still carry no limiter). It adds one frame the
