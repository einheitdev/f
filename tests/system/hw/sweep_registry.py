...
SCENARIOS = {s.name: s for s in [
    ...
    # ---------------- Layer 7: tier 2 ------------------------------
    # The ID keeps the word 'gap' although the gap closed on
    # 2026-08-19: the sweep history is keyed by it, and renaming
    # resets the record of whether this check has ever gone red.
    # Both halves are plantable now, which they could not be while
    # the Tier 2 side asserted a compiler stub.
    _s('l7_01_tier2_rate_limit_gap',
       'the Tier 1 rate_limit MODIFIER and the Tier 2 CALL form both cap '
       'the same flood, and cap it the same way',
       [_p('tier1-budget-infinite',
           ...
           [PolicySub(tag='l7-01a', regex=True,
                      ...
                      repl='rate_limit(1000000, per=src_ip)')],
           residual=''),
        _p('tier2-budget-infinite',
           'the Tier 2 call\'s budget goes to a million, which is what '
           'the old constant-false stub was indistinguishable from',
           [PolicySub(tag='l7-01b', regex=True,
                      find=r'rate_limit\((?:\d+|\$\w+), per=src_ip\)',
                      repl='rate_limit(1000000, per=src_ip)')],
           residual='')],
       timeout_s=300),
